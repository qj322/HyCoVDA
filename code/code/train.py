...
def split_train_validation(train_pos: np.ndarray, fold_id: int,
    ...
    """Deterministically split a fold's training positives without using its test set."""
    ...
    n_val = max(1, int(round(len(train_pos) * validation_ratio)))
    n_val = min(n_val, len(train_pos) - 1)
    val_idx, fit_idx = order[:n_val], order[n_val:]
    return train_pos[fit_idx], train_pos[val_idx]


def train_one_fold(model: HyCoVDA,
                   train_adj,
                   train_pos,
                   val_pos,
                   global_pos_pairs,
    ...
    val_neg_fixed = sample_fixed_negatives(model.D, model.V, global_pos_set,
                                           len(val_pos) * TEST_NEG_PER_POS,
                                           seed=SEED + 1000 * fold_id) if len(val_pos) else np.empty((0, 2), dtype=np.int64)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        val_auc, val_aupr, val_f1, val_precision, val_recall, val_acc = eval_on_pairs(
            model, val_pos, model.D, model.V,
            fixed_neg_pairs=val_neg_fixed, device=device
        )
        
        print(f"[Fold {fold_id}] Ep {ep:03d} loss={total_loss/max(1,steps):.4f}  Val AUC={val_auc:.4f} AUPR={val_aupr:.4f} F1={val_f1:.4f}")
...
def main():
    ...
    for fold_id, (train_pos, test_pos) in enumerate(folds, start=0):
        ...
        print(f"[Fold {fold_id}] fit={len(fit_pos)} val={len(val_pos)} test={len(test_pos)}")
        ...
        best_path = train_one_fold(model, train_adj, fit_pos, val_pos,
        ...
        print(f"[Fold {fold_id}] Selected checkpoint Test AUC={test_auc:.4f}  AUPR={test_aupr:.4f}  F1={test_f1:.4f}  -> {best_path}")
